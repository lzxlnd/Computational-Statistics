...
D<-function(x,y){       #x: observations, y: distribution
  ...
  for (i in 1:n) {
    f[i]<-abs(as.numeric( i/n-rank(c(x[i],sy))[1]/(m+1))) #calculate difference of p
...
MC.KS<-function(x,y,times,size=100,probs =.05,plot=FALSE){       #y is a vector, size: sample size, times: sample times
...
MC.KS(precip,qgamma(seq(length(precip))/(1+length(precip)),.377,1.684),100,size = 100 ,plot = TRUE)
ks.test(precip,qgamma(seq(length(precip))/(1+length(precip)),.377,1.684))
